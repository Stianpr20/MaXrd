...
InputCheck[input_List, labels___ ? (SubsetQ[{"1hkl", "1xyz", "Integer",
     "Multiple", "StringSymbol", "WrapSingle"}, {#}]&)] :=
    Block[
        {check, hkl, temp}
        ,
        (* Check labels *)
        ...
        Do[
            temp = check[[i]];
            If[!MemberQ[{"1hkl", "1xyz", "Integer", "Multiple", "StringSymbol",
                 "WrapSingle"}, temp],
        ...
        Which[(* Single reflection/coordinate *)
            MatchQ[input, {x1_, x2_, x3_} /; !AnyTrue[{x1, x2, x3}, ListQ
                ...
                hkl = {input}
            ,
            (* Multiple reflections/coordinates *)AllTrue[input, MatchQ[
                #, {x1_, x2_, x3_} /; !AnyTrue[{x1, x2, x3}, ListQ]]&],
                hkl = input
            ,
        ...
        If[MemberQ[check, "StringSymbol"],
            If[!ContainsAll[{Integer, String, Symbol, Times}, Head /@
                 Flatten @ hkl],
        ...
        If[MemberQ[check, "WrapSingle"],
            If[MatchQ[input, {_ ? (!ListQ[#]&), _ ? (!ListQ[#]&), _ ?
                 (!ListQ[#]&)}],
                Return[{input}]
                ,
                Return[input]
...
InputCheck["GenerateTargetPositions", {x_, y_, z_}] :=
    Flatten[Table[{i, j, k}, {i, 0., N @ x - 1}, {j, 0., N @ y - 1}, 
        {k, 0., N @ z - 1}], 2]
...
InputCheck["GetAtomData", label_String] :=
    Block[{data},
        ...
        If[MissingQ @ data,
            data = Lookup[StianRamsnes`MaXrd`Private`$TempCrystalData,
                 label]
        ];
        data["AtomData"]
    ]
    ...
    Block[{group, letter, vectors},
        If[StringLength @ centring =!= 1,
            group = InputCheck["GetPointSpaceGroupCrystal", centring]
                ;
            letter = First @ StringCases[ToString[group, OutputForm],
                 LetterCharacter];
...
InputCheck["GetCrystalFamilyMetric", family_, dimension_] :=
    Block[
        ...
        ,
        (* Input checks *)
        If[!MemberQ[{"Cubic", "Hexagonal", "Tetragonal", "Orthorhombic",
             "Monoclinic", "Triclinic"}, family],
        ...
        (* Metric *)
        M = {{1, b Cos[\[Gamma]], c Cos[\[Beta]]}, {0, b Sin[\[Gamma]
            ], c (Cos[\[Alpha]] - Cos[\[Beta]] Cos[\[Gamma]]) Csc[\[Gamma]]}, {0,
             0, c * Sqrt[1 - Cos[\[Beta]] ^ 2 - (Cos[\[Alpha]] - Cos[\[Beta]] Cos[
            \[Gamma]]) ^ 2 Csc[\[Gamma]] ^ 2]}};
        M =
            N[
                M /.
                    Switch[family,
                        "Cubic",
                            {a -> 1., b -> 1., c -> 1., \[Alpha] -> 90
                                 * \[Degree], \[Beta] -> 90 \[Degree], \[Gamma] -> 90 \[Degree]}
                        ,
                        "Hexagonal",
                            {a -> 1., b -> 1., c -> 1., \[Alpha] -> 90
                                 * \[Degree], \[Beta] -> 90 \[Degree], \[Gamma] -> 120 \[Degree]}
                        ,
                        "Tetragonal",
                            {a -> 1., b -> 1., c -> 1.61803, \[Alpha]
                                 -> 90 \[Degree], \[Beta] -> 90 \[Degree], \[Gamma] -> 90 \[Degree]}
                        ,
                        "Orthorhombic",
                            {a -> 1.7, b -> 1.2, c -> 0.85, \[Alpha] 
                                -> 90 \[Degree], \[Beta] -> 90 \[Degree], \[Gamma] -> 90 \[Degree]}
                        ,
                        "Monoclinic",
                            {a -> 1., b -> 0.7, c -> 1.2, \[Alpha] ->
                                 90 \[Degree], \[Beta] -> 72. \[Degree], \[Gamma] -> 90 \[Degree]}
                        ,
                        "Triclinic",
                            {a -> 1.3, b -> 0.8, c -> 0.9, \[Alpha] ->
                                 66. \[Degree], \[Beta] -> 77. \[Degree], \[Gamma] -> 88. \[Degree]}
                    ]
            ];
        If[dimension === "2D",
            M[[{1, 2}, {1, 2}]]
...
InputCheck["GetCrystalFormulaUnits", input_String] :=
    Block[
        ...
        If[KeyExistsQ[$CrystalData[input], "FormulaUnits"],
            ...
            output = ToExpression @ InputString["Cannot determine the number of formula units "
...
(* Convert wavelength or energy to \[ARing]ngstr\[ODoubleDot]ms 
    *)Which[
                    ...
                    CompatibleUnitQ[input, "Joules"],
                        \[Lambda] = hcKeV / QuantityMagnitude @ UnitConvert[
                            input, "Kiloelectronvols"]
                    ,
                    True,
...
InputCheck["GetReciprocalImageOrientation", latticeInput_, hklPlane_,
     indexLimit_, {width_, height_}, directSpaceQ_] :=
    Block[{hkl = hklPlane, abscissaIndex, ordinateIndex, planeConstant,
         planeIndex, bottomLeft = {-1, -1}, bottomRight = {1, -1}, topRight =
         {1, 1}, topLeft = {-1, 1}, M, \[Xi], imageOrientation},
        If[StringQ @ hkl,
            hkl = MillerNotationToList @ hkl
        ];
        {abscissaIndex, ordinateIndex, planeConstant} = {#1, #2, hkl[[
            #3]]}& @@ Flatten[Position[hkl, #]& /@ {"h", "k", "l", _Integer}];
        ...
        If[TrueQ @ directSpaceQ,
            ...
            imageOrientation = Insert[#, N @ planeConstant, planeIndex
                ]& /@ imageOrientation;
            imageOrientation = Append[#1, #2]& @@@ Transpose[{imageOrientation,
                 {width, height, 1}}];
            PrependTo[imageOrientation, imageOrientation[[3, {1, 2, 3
                }]]]
            ,
            (* b. Corners in reciprocal space *)
            imageOrientation = indexLimit * {bottomLeft, bottomRight,
                 topLeft};
            imageOrientation = N @ Insert[#, planeConstant, planeIndex
                ]& /@ imageOrientation
        ];
        imageOrientation = MapAt[#, imageOrientation, {All, {abscissaIndex,
             ordinateIndex}}]&[DecimalForm[#, {7, 4}, NumberPadding -> {" ", "0"}
            ]&];
...
InputCheck["InterpretElement", input_] :=
    Block[
        {elementsIn = input, periodicTable, elementsRead, elementsReadNeutral,
             temp}
        ,
        (*---* A. Input number *---*)
        ...
        If[NumericQ @ elementsIn,
            If[(1 <= elementsIn <= Length @ $PeriodicTable) && IntegerQ
                 @ elementsIn,
        ...
        If[MemberQ[elementsRead, {}],
            Message[InputCheck::ElementError, Part[elementsIn, Position[
                elementsRead, {}][[1, 1]]]];
            Abort[]
        ];
        ...
        elementsReadNeutral = elementsReadNeutral /. s_String /; !MemberQ[
            periodicTable, s] :> StringTake[s, 1];
        ...
        If[temp =!= {},
            Message[InputCheck::ElementError, Part[elementsIn, Position[
                elementsReadNeutral, First @ temp][[1, 1]]]];
            Abort[]
        ];
        ...
        If[StringContainsQ[sg, "origin", IgnoreCase -> True],
            ...
            If[o === {},
                ...
                o = ChoiceDialog["Information on cell origin detected."
                     <> "\n" <> "Please confirm the cell origin.", {1, 2}, WindowTitle ->
                     "Cell origin"];
...
*)StartOfString ~~ {"'", "\""} ~~ main__ ~~ {"'", "\""} ~~ EndOfString
     :> main
                    ,
                    (* B.4.2 Uppercase centring 1 *)
                    StartOfString ~~ first : {"-" ~~ _, _} ~~ rest__ 
                        ~~ EndOfString :> ToUpperCase @ first ~~ ToLowerCase @ rest
                    ,
                    (* B.4.3. Fix boxes *)
                    {"overscriptbox" -> "OverscriptBox", "subscriptbox"
                         -> "SubscriptBox"}
                    ,
                    ...
                    ,
                    (* B.4.5 Screw axes *)
                    a:DigitCharacter ~~ "(" ~~ b:DigitCharacter ~~ ")"
                         :> a <> b
                    ,
        ...
        If[!MemberQ[{"P", "I", "F", "rotationTransform", "A", "B", "C",
             "H", "\!", "-"}, StringTake[sg, 1]],
            Goto["Failed"]
        ];
...
InputCheck["PadDomain", {{sizeA_, sizeB_, sizeC_}, domain_List}, padding_Integer
    :1] :=
    Block[{newSize, contentPositions, contentMap, voidPositions, voidMap,
         joined},
        If[padding <= 0,
            Return[{{sizeA, sizeB, sizeC}, domain}]
        ];
        newSize = (padding + 1) {sizeA, sizeB, sizeC} - padding;
        contentPositions = Flatten[Table[{i, j, k}, {i, 0., (padding 
            + 1) * sizeA - 1, padding + 1}, {j, 0., (padding + 1) * sizeB - 1, padding
             + 1}, {k, 0., (padding + 1) * sizeC - 1, padding + 1}], 2];
        contentMap = AssociationThread[contentPositions -> domain];
        voidPositions = Complement[InputCheck["GenerateTargetPositions",
             newSize], contentPositions];
...
InputCheck["PointGroupQ", input_String] :=
    (
        (* Check if valid point group string *)If[!MemberQ[$PointGroups,
             input, Infinity],
...
InputCheck["PointSpaceGroupQ", input_String] :=
    (* Check if valid point- or space group string *)If[!KeyExistsQ[$GroupSymbolRedirect,
         input],
...
InputCheck["ProcessWavelength", crystal_String, wavelength_, abortQ_:
    True] :=
    Block[{\[Lambda]},
        \[Lambda] =
            If[N @ wavelength === -1.,
...
InputCheck["RecognizeFractions", coordinates_List, threshold_:0.001] :=
    Block[{recognizedFractions = {1/12, 1/8, 1/6, 1/4, 1/3, 3/8, 5/12,
         1/2, 7/12, 5/8, 2/3, 3/4, 5/6, 7/8, 11/12}, rationalized, matchedQ, 
        Replacer},
        rationalized = Map[Rationalize[#, threshold]&, coordinates, {
            -1}];
        matchedQ = Map[MemberQ[Prepend[recognizedFractions, 0], #]&, 
            rationalized, {-1}];
        Replacer[x_, p_] :=
...
InputCheck["RotationTransformation", {{sizeA_Integer, sizeB_Integer, 
    sizeC_Integer}, domains_List}, {anchorShiftInput_List, anchorReference_String,
     rotations_, rotationAxes_ : IdentityMatrix @ 3}, cellDimensionsInput_List
     : {1., 1., 1.}, force3DimensionalInterpretation_:False] :=
    Module[
        {twoDimensionalQ, coordinates, \[Zeta], rotationTransform, uniqueDomains,
             anchorShift = anchorShiftInput, anchors, cellDimensions = cellDimensionsInput,
             coordinatesGrouped, domainAnchors, zeroRotation, zeroAnchor}
        ,
        ...
        If[AssociationQ @ rotations,
            If[twoDimensionalQ,
                ...
                ,
                If[AnyTrue[Values @ rotations, !MatchQ[#, ConstantArray[
                    _?NumericQ, 3]]&],
        ...
        If[anchorReference === "Domain" || anchorReference === "DomainCentroid",
            ...
            coordinates = Flatten[Table[{i, j, k}, {i, 0., sizeA - 1},
                 {j, 0., sizeB - 1}, {k, 0., sizeC - 1}], 2];
            If[twoDimensionalQ,
            ...
            domainAnchors =
                <|
                        "Domain" -> Flatten[TakeSmallestBy[#, Norm, 1
        ...
        (* Rotation transformation function 'rotationTransform' *)
        ...
        Which[
            anchorReference === "Host" && twoDimensionalQ,
                rotationTransform[d_] := Chop @ RotationTransform[\[Zeta]
                     @ d, anchorShift]
            ,
            anchorReference === "Host",
                rotationTransform[d_] := Chop[Composition @@ MapThread[
                    RotationTransform[#1, #2, anchorShift]&, {\[Zeta] @ d, rotationAxes}]
                    ];
            ...
            anchorReference === "Unit" && twoDimensionalQ,
                rotationTransform[d_, p_] := Chop @ RotationTransform[
                    \[Zeta] @ d, p + anchorShift]
            ,
            ...
            twoDimensionalQ,
                rotationTransform[d_] := Chop @ RotationTransform[\[Zeta]
                     @ d, Lookup[anchors, d, zeroAnchor] + anchorShift]
            ,
            True,
                rotationTransform[d_, p_] := Chop[Composition @@ MapThread[
                    RotationTransform[#1, #2, Lookup[anchors, d, zeroAnchor] + anchorShift
                    ]&, {\[Zeta] @ d, rotationAxes}]]
        ];
        rotationTransform
    ]
...
InputCheck["Update$CrystalDataAutoCompletion"] :=
    (
        FE`Evaluate[FEPrivate`AddSpecialArgCompletion[#]]&["$CrystalData"
             -> {Keys @ $CrystalData, {"AtomData", "ChemicalFormula", "FormulaUnits",
             "LatticeParameters", "Notes", "SpaceGroup", "Wavelength"}}];
        $CrystalData = KeySort @ $CrystalData;
        Keys @ $CrystalData
    )

InputCheck["Update$CrystalDataFile", dataFile_String, newStructureLabel_String,
     hostCopy_] :=
    (
