...
Options[StructureFactorTable] =
    {
        (* DarwinWidth and ExtinctionLength options *)"Polarisation" 
            -> "\[Pi]"
        ,
        ...
        ,
        "SubtitleStyle" -> {FontFamily -> "Times New Roman", FontSize
             -> 13, Gray}
        ,
...
SyntaxInformation @ StructureFactorTable = {"ArgumentsPattern" -> {_,
     _., _., OptionsPattern[]}};

Begin["`Private`"];

StructureFactorTable[crystal_String, lambda : _ ? (NumericQ[#] || QuantityQ[
    #]&) : -1, condition___Condition, OptionsPattern[]] :=
    Block[
        {\[Lambda], hkl, H, sl, bragg, column1, column2, column3, column4,
             column5, column6, V, zeros, R, \[Theta], Fh, FhBar, \[CapitalLambda]o,
             \[Delta]os, temp, temp1, temp2, temp3, temp4, temp5, temp6, sort, keep,
        ...
        hkl = Check[ReflectionList[crystal, \[Lambda], condition, "SplitEquivalent"
             -> OptionValue["SplitEquivalent"], "CustomReflections" -> OptionValue[
            "CustomReflections"], "Keep" -> OptionValue["ReflectionListKeep"]], Abort[
            ]];
        (* Useful variables *)
        ...
        (* Structure factors *)
        temp = Check[StructureFactor[crystal, hkl, \[Lambda], "Threshold"
             -> threshold], Abort[]];
        ...
        R = QuantityMagnitude @ UnitConvert[Quantity["ClassicalElectronRadius"
            ], "Angstroms"];
        \[Theta] = column4 * Degree;
        Fh = column2;
        FhBar = (StructureFactor[crystal, -hkl, \[Lambda]])[[All, 1]]
            ;
        \[CapitalLambda]o = Quiet @ ExtinctionLength[crystal, \[Lambda],
             hkl, "Units" -> False, "Polarisation" -> polarisation];
        \[CapitalLambda]o = ReplacePart[\[CapitalLambda]o, zeros -> "\[Dash]"
            ];
        (* Darwin width *)
        \[Delta]os = Quiet @ DarwinWidth[crystal, \[Lambda], hkl, "Units"
             -> False, "Polarisation" -> polarisation];
        ...
        temp2 = Sort[Transpose @ temp1, #1[[Abs @ sort]] < #2[[Abs @ 
            sort]]&];
        ...
        temp3 = NumberForm[#, {5, 3}, DigitBlock -> 3, NumberSeparator
             -> ","]& /@ (Flatten @ temp2);
        temp4 = Partition[temp3, Last @ Dimensions @ temp2];
        (*---* Table construction *---*)
        temp5 = PrependTo[temp4, {"(hkl)", "|\!\(\*SubscriptBox[\(F\), \(hkl\)]\)|",
             "\!\(\*SubscriptBox[\(\[Phi]\), \(hkl\)]\) [\[Degree]]", "\!\(\*SubscriptBox[\(\[Theta]\), \(B\)]\) [\[Degree]]",
             "\!\(\*SubscriptBox[\(\[CapitalLambda]\), \(0\)]\) [\[Micro]m]", "2\!\(\*SubscriptBox[\(\[Delta]\), \(os\)]\) [\[Micro]rad]"
            }];
        temp6 = PrependTo[temp5, {Null, "Structure factor", "Phase", 
            "Bragg angle", "Extinction length", "Darwin width"}];
        Grid[temp6, Dividers -> OptionValue[Dividers], Background -> 
            OptionValue[Background], Alignment -> {Center, Center}, Spacings -> {
            Automatic, 2 -> 0.5}, ItemStyle -> {Automatic, Automatic, {{{1, 1}, {
            1, First @ Dimensions @ temp1}} -> OptionValue["TitleStyle"], {{2, 2},
             {1, First @ Dimensions @ temp1}} -> OptionValue["SubtitleStyle"], {{
