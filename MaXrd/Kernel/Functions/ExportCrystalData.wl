...
Options @ ExportCrystalData = {"Detailed" -> False, "IncludeStructureSizeInfo"
     -> True};

SyntaxInformation @ ExportCrystalData = {"ArgumentsPattern" -> {_, _,
...
ExportCrystalData["DISCUS", crystal_String, outputFile_String, OptionsPattern[
    ...
    Block[
        {crystalData, atomData, size, unitCellAtomCount, latticeParameters,
             formatter, appendComma, simpleQ, preambleTitle, preambleSpaceGroup, 
            preambleCell, preambleCount, preambleAtomsHeader, preamble, elements,
             coordinates, displacementParameters, items, spacesToUse, additional,
             makeSpace, atoms}
        ,
        (* Loading necessary data *)
        {crystalData, atomData, size, latticeParameters} = ECD$LoadNecessaries
             @ crystal;
        unitCellAtomCount = Lookup[crystalData["Notes"], "UnitCellAtomsCount",
             Round[Length[atomData] / Times @@ size]];
        (* Auxiliary *)
        formatter[x_] := ToString @ NumberForm[N @ Chop[x, Power[10.,
             -5]], {9, 6}];
        ...
        preambleSpaceGroup = {"spcgr  ", StringDelete[$GroupSymbolRedirect[
            crystalData["SpaceGroup"]]["Name", "HermannMauguinShort"], " "]};
        preambleCell = {"cell   ", StringJoin @ Riffle[latticeParameters,
             "  "]};
        preambleCount = {"ncell  ", StringJoin @ Riffle[ToString /@ Join[
            size, {unitCellAtomCount}], ",  "]};
        preambleAtomsHeader =
            If[simpleQ,
                {"atoms\n"}
                ,
        ...
        displacementParameters = TransformAtomicDisplacementParameters[
        ...
        spacesToUse = Transpose @ ConstantArray[{11, 16, 16, 15, 8}, 
            Length @ atomData];
        additional = "1,       0,       0,   1.000000";
        Do[spacesToUse[[i]] = spacesToUse[[i]] - (StringLength /@ items
            [[i]]), {i, Length @ items - 1}];
...
ExportCrystalData["DIFFUSE", crystal_String, outputDir_String, hklPlane_,
     indexLimit_, subtractionMode_String, {width_Integer, height_Integer},
     OptionsPattern[]] :=
    Block[
        {reorganise, source, scatteringFactorTemplate, crystalData, atomData,
             allElements, size, latticeParameters, directionCosines, M, partA, X,
             Y, Z, unitCells, MakeSitesTable, table, sitesList, partB, maxPossibleSites,
             newAtomData, blockLengths, headerComments, headerData, padWidth, header,
             scatteringData}
        ,
        (* Checks *)
        ...
        reorganise[data_Association] :=
            With[{xyz = data["FractionalCoordinates"]},
                {Min /@ Transpose[{IntegerPart @ xyz + 1, size}], StringJoin[
                    "   ", StringPadRight[ToString @ DecimalForm[#, {11, 5}]& /@ Mod[xyz,
                     1], 11], StringDelete[data["Element"], {DigitCharacter, "+", "-"}]]}
        ...
        source = Import @ FileNameJoin[{$MaXrdPath, "Kernel", "Data",
             "AtomicScatteringFactor", "InternationalTablesC(3rd).m"}];
        scatteringFactorTemplate[element_] := StringTemplate["'`X`'
`a1`,`b1`,`a2`,`b2`
`a3`,`b3`,`a4`,`b4`,`c`
0.0,0.0"
            ] @ Join[source[element], <|"X" -> element|>];
        (* Loading necessary data *)
        {crystalData, atomData, size, latticeParameters} = ECD$LoadNecessaries
             @ crystal;
        allElements = DeleteDuplicates @ atomData[[All, "Element"]];
        allElements = DeleteDuplicates[StringDelete[#, {DigitCharacter,
             "+", "-"}]& /@ allElements];
        directionCosines = ToString @ DecimalForm[Cos[# * Degree], {6,
             5}]& /@ N @ latticeParameters[[4 ;; 6]];
        ...
        partB = Flatten @ Riffle[StringTemplate[" `1` `2` `3`"] @@@ newAtomData
            [[All, 1, 1]], newAtomData[[All, All, 2]]];
        (* Part A *)
        sitesList = MakeSitesTable /@ blockLengths;
        {X, Y, Z} = size;
        unitCells = Flatten[Table[StringTemplate[" `1` `2` `3`"][x, y,
             z], {x, 1, X}, {y, 1, Y}, {z, 1, Z}], 2];
        ...
        headerComments = {"Header or structure label", "Random number seeds",
             "Cell coord's; angles are cos(ang)", "Size of crystal simulation (unit cells)",
             "Periodic Boundary?", "Origin of volume", "u-axis (bottom right) and image x-dimension",
             "v-axis (top left) and image y-dimension", "w-axis (top right) and image z-dimension",
             "sin(theta)/lambda maximum", "Lot size", "Number of lots", "Number of atom sites per cell",
             "Number of different atom types (list of sc.coef's below)", "Subtract average lattice? ('N', 'e', 'E' or 'Y')"
            };
        headerData = {crystal, "12645, 9676", StringTemplate["`1` `2` `3`  `4` `5` `6`"
            ] @@ Join[latticeParameters[[1 ;; 3]], directionCosines], StringTemplate[
            "`1` `2` `3`"] @@ size, "Y", Sequence @@ InputCheck["GetReciprocalImageOrientation",
             crystal, hklPlane, indexLimit, {width, height}, True], "3.0", StringTemplate[
            "`1` `2` `3`"] @@ size, "1", ToString @ maxPossibleSites, ToString @ 
            Length @ allElements, subtractionMode};
        ...
        (* Prepare output and export *)
        {Export[FileNameJoin[{outputDir, "diffuse_input1_crystal.txt"
            }], Join[partA, partB, {"\n"}], "Table"], Export[FileNameJoin[{outputDir,
             "diffuse_input2_setup.txt"}], Join[header, scatteringData, {"\n"}], 
            "Table"]}
    ]
...
ECD$LoadNecessaries[crystal_String] :=
    Block[{crystalData, atomData, crystalNotes, size, latticeParameters
        ...
        size = Lookup[crystalNotes, "StructureSize", Round /@ Max /@ 
            Transpose @ atomData[[All, "FractionalCoordinates"]]] /. {0 -> 1};
        latticeParameters = GetLatticeParameters[crystal, "Units" -> 
            False];
