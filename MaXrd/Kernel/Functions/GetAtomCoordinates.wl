...
Options @ GetAtomCoordinates = {"Cartesian" -> False, "GatherElements"
     -> True, "IgnoreCharge" -> True};
...
GetAtomCoordinates[crystalObject_, options : OptionsPattern[]] :=
    Block[{atomData, plotData, toCartesianQ = TrueQ @ OptionValue["Cartesian"
        ], gatherElementsQ = TrueQ @ OptionValue["GatherElements"], ignoreChargeQ
         = TrueQ @ OptionValue["IgnoreCharge"], transformationMatrix},
        Switch[crystalObject,
            _String,
                InputCheck["CrystalQ", crystalObject];
                atomData = Lookup[InputCheck["GetAtomData", crystalObject
                    ], {"Element", "FractionalCoordinates"}];
                If[gatherElementsQ,
                    atomData = Merge[MapThread[Rule, Transpose @ atomData
                        ], Identity]
                ];
                If[toCartesianQ,
                    transformationMatrix = GetCrystalMetric[crystalObject,
                         "ToCartesian" -> True];
                    atomData = GAC$TransformCoordinates[transformationMatrix,
                         atomData]
                ]
            ,
            _Graphics3D,
                plotData = Cases[First @ crystalObject, x_ /; Head @ 
                    x[[2]] === Opacity];
                ...
                If[!gatherElementsQ,
                    atomData = List @@@ Flatten @ KeyValueMap[Thread[
                        #1 -> #2]&, atomData]
                ];
                If[!toCartesianQ,
                    transformationMatrix = Inverse @ GAC$GetMetricFromPlotVectors
                         @ crystalObject;
                    atomData = GAC$TransformCoordinates[transformationMatrix,
                         atomData]
                ]
        ...
        If[ignoreChargeQ,
            atomData = atomData /. x_String :> StringReplace[x, RegularExpression[
                "([A-Z][a-z]?).*"] -> "$1"]
        ];
        atomData
    ]
