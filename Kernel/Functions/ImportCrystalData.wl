...
Options @ ImportCrystalData = {"DataFile" -> FileNameJoin[{$MaXrdPath,
     "Kernel", "Data", "UserData", "CrystalData.m"}], "ExtractSubData" ->
     1, "IgnoreIonCharge" -> False, "Notes" -> <||>, "RoundAnglesThreshold"
     -> 0.001, "Units" -> True, "OverwriteWarning" -> True};

SyntaxInformation @ ImportCrystalData = {"ArgumentsPattern" -> {___, 
    OptionsPattern[]}};

Begin["`Private`"];

ImportCrystalData[{crystalName_String, chemicalFormula : _?StringQ : 
    "", Z : _?IntegerQ : 0, spaceGroup_:1, wavelength : _?NumericQ : -1},
     getLatticeParameters_List, atomData_List, OptionsPattern[]] :=
    Block[
        {choice, name, sg, cell, \[Delta], fr, latticeItem, \[Lambda],
             itemAtomData, item, dataFile = OptionValue["DataFile"], temp}
        ,
        ...
        (*---* Lattice parameters *---*)
        If[!AllTrue[getLatticeParameters, NumericQ[#]&] || Length @ getLatticeParameters
             != 6,
        ...
        latticeItem = Association @ Thread[{"a", "b", "c", "\[Alpha]",
             "\[Beta]", "\[Gamma]"} -> cell];
        (*---* Wavelength *---*)
        (* Optional: Use units *)
        If[OptionValue["Units"],
        ...
        If[atomData === {},
            itemAtomData = {<||>}
            ,
            itemAtomData = Table[DeleteMissing @ Part[atomData[[i]], 
                {"Element", "OccupationFactor", "SiteSymmetryMultiplicity", "SiteSymmetryOrder",
                 "FractionalCoordinates", "DisplacementParameters", "Type"}], {i, Length
                 @ atomData}];
            Do[
                If[KeyExistsQ[First @ itemAtomData, k],
                    itemAtomData = MapAt[ToExpression, itemAtomData, 
                        {All, Key[k]}]
                ]
                ,
                {k, {"OccupationFactor", "SiteSymmetryMultiplicity", 
                    "SiteSymmetryOrder", "DisplacementParameters"}}
            ];
            (* Checking strings in coordinates *)
            Do[itemAtomData[[i, "FractionalCoordinates"]] = itemAtomData
                [[i, "FractionalCoordinates"]] /. x_String :> ToExpression[x], {i, Length
                 @ itemAtomData}]
        ];
        (*---* Preparing item *---*)
        item = <|"ChemicalFormula" -> chemicalFormula, "FormulaUnits"
             -> Z, "SpaceGroup" -> sg, "LatticeParameters" -> latticeItem, "Wavelength"
             -> \[Lambda], "AtomData" -> itemAtomData, "Notes" -> OptionValue["Notes"
            ]|>;
        (* Delete certain keys *)
        ...
        If[AllTrue[N @ temp, # == 1.&],
            item[["AtomData", All]] = KeyDrop[item[["AtomData", All]],
                 "OccupationFactor"]
        ];
...
ImportCrystalData[cifFile_, inputName_String:"", OptionsPattern[]] :=
    Block[
        ...
        temp = StringCases[import, Shortest[{"_diffrn_radiation_type",
             "_diffrn_radiation_probe"} ~~ Whitespace ~~ {"", enc} ~~ t : LetterCharacter
            .. ~~ "\n"] :> t];
        If[MemberQ[temp, #],
        ...
        cell = StringCases[import, Shortest[x : ("_cell_" ~~ {"length",
             "angle"} ~~ __ ~~ {DigitCharacter, "."}..) ~~ {"(", Whitespace}] :> 
            ToLowerCase @ x, IgnoreCase -> True];
        (* Check *)
        If[cell == {},
            Message[ImportCrystalData::latticeParameters];
            Abort[]
        ...
        (* Correct ordering *)
        cell = Partition[cell, Length @ cell / Quotient[Length @ cell,
             6]];
        Do[
            X = cell[[i]];
            x = X[[All, 1]];
            P = FindPermutation[x, {"_cell_length_a", "_cell_length_b",
        ...
        atomData = StringCases[import, Shortest[labels : (Whitespace 
            ~~ "_atom_site_" ~~ __ ~~ "\n") ~~ data : (StartOfLine ~~ Whitespace...
             ~~ LetterCharacter ~~ __ ~~ "\n") ~~ {endString, "_atom_site_aniso",
             "_" ~~ Except["a"]}] :> {labels, data}];
        (* No data? *)
        If[atomData == {},
            Message[ImportCrystalData::atomData];
            Abort[]
        ...
        atomData = DeleteCases[atomData, x_ /; StringContainsQ[x[[1]],
             "aniso"]];
        (* Specify sub-data *)
        atomData = atomData[[sub]];
        (* C.2. Organising data *)
        atomTags = Flatten @ StringCases[atomData[[1]], "_atom_site_"
             ~~ {WordCharacter, "_"}..];
        ...
        If[!KeyExistsQ[atomData, "_atom_site_adp_type"],
            AppendTo[atomData, "_atom_site_adp_type" -> ConstantArray[
                "Uiso", L]]
        ];
        If[!KeyExistsQ[atomData, "_atom_site_U_iso_or_equiv"],
            AppendTo[atomData, "_atom_site_U_iso_or_equiv" -> ConstantArray[
                0, L]]
        ];
        (* D.2. Anisotropic displacement parameters *)
        anisotropicData = StringCases[import, Shortest["loop_" ~~ Whitespace
        ...
        anisotropicOrder = Flatten @ StringCases[anisotropicData, "U_"
             ~~ DigitCharacter..];
        P = FindPermutation[{"U_11", "U_22", "U_33", "U_12", "U_13", 
            "U_23"}, anisotropicOrder];
        (* Nothing there to extract? *)
        temp = Flatten @ StringCases[anisotropicData, Shortest["_atom_site_aniso_"
             ~~ x__ ~~ Whitespace] :> x];
        c = Flatten @ Quiet @ Position[temp, x_ /; StringContainsQ[x,
             "U"]];
...
Last line *)
        ...
        anisotropicData = StringReplace[anisotropicData, x__ ~~ "(" ~~
             __ ~~ ")" :> x];
        anisotropicData = Partition[anisotropicData, Length @ temp];
        (* Correcting parameter order *)
        anisotropicData[[All, c]] = Permute[#, P]& /@ anisotropicData
            [[All, c]];
        (* Associating each atom with values *)
        anisotropicData = Association[Table[anisotropicData[[i, 1]] ->
             anisotropicData[[i, c]], {i, Length @ anisotropicData}]];
        ...
        Do[
            item = <||>;
            AppendTo[item, "Element" -> atomData[["_atom_site_type_symbol",
                 i]]];
            Do[
                If[KeyExistsQ[atomData, tags[[j]]],
                    AppendTo[item, labels[[j]] -> atomData[[tags[[j]],
                         i]]]
                ]
                ,
                {j, Length @ tags}
            ];
            AppendTo[item, "FractionalCoordinates" -> Evaluate[atomData
                [["_atom_site_fract_" <> #, i]]& /@ {"x", "y", "z"}]];
            If[StringTake[atomData[["_atom_site_adp_type", i]], -3] ===
                 "ani",
                displacementParameters = Part[anisotropicData, atomData
                    [["_atom_site_label", i]]]
                ,
                displacementParameters = atomData[["_atom_site_U_iso_or_equiv",
                     i]]
            ];
            AppendTo[item, "DisplacementParameters" -> displacementParameters
                ];
            AppendTo[item, "Type" -> atomData[["_atom_site_adp_type",
                 i]]];
        ...
        (* E.2. Forumla units (Z) *)
        Z = StringCases[import, Shortest["_cell_formula_units_Z" ~~ Whitespace
             ~~ z:DigitCharacter] :> z];
        If[Z === {},
            Z = 0
            ,
        ...
        formula =
            StringCases[import, Shortest[# ~~ {Whitespace, "\n"}.. ~~
                ...
                {
                    (* Prioritised order *)"_chemical_formula_iupac",
        ...
        If[AnyTrue[formula, StringContainsQ[#, {"(", ")"}]&],
            {l, r} = Flatten @ Position[StringPosition[formula, {"(",
                 ")"}], {{_, _}}];
            checkParentheses = True
        ];
        chemicalFormula = {};
        Do[
            temp = Flatten @ StringCases[formula[[i]], x : LetterCharacter
            ...
            Which[
                ...
                ,
                True,
                    AppendTo[chemicalFormula, left <> temp[[1]] <> mid
                         <> temp[[2]] <> right]
            ]
            ,
            {i, Length @ formula}
        ...
        sgTags = {"_space_group_name_Hall", "_space_group_name_H-M_alt",
             "_space_group_IT_number", "_symmetry_space_group_name_Hall", "_symmetry_space_group_name_H-M",
        ...
        sgData = StringCases[sgData, t:sgTags ~~ Whitespace ~~ sg : Shortest[
            Except[WhitespaceCharacter] ~~ __] ~~ {"\n", EndOfString} :> {t -> sg
            }];
        ...
        If[sg === Null && modulationQ,
            sg = StringCases[import, "_space_group_ssg_name" ~~ Whitespace
                 ~~ {"'", "\""} ~~ sg__ ~~ {"'", "\""} ~~ "\n" :> sg];
            sg = StringCases[sg, Shortest[StartOfString ~~ ___ ~~ sg 
                : (LetterCharacter ~~ __) ~~ "("] :> sg];
            If[sg =!= {},
                sg = Quiet @ InputCheck["InterpretSpaceGroup", First 
                    @ Flatten @ sg, False]
            ]
        ];
(* G.6. If missing space group, display message and use 'P1' 
    *)
        ...
        options = Thread[# -> OptionValue[#], String]& /@ (First /@ Options
             @ ImportCrystalData);
        ImportCrystalData[{name, chemicalFormula, Z, sg, \[Lambda]}, 
            cell, atomOverview, options]
    ]
...
ImportCrystalData["RunDialogue"] :=
    DialogInput[
        DynamicModule[
            ...
            (*---* Setup for grid B *---*)
            (* Logic *)
            elementListWithNumber = # <> " (" <> ToString @ $PeriodicTable[
                #, "AtomicNumber"] <> ")"& /@ Keys @ $PeriodicTable;
            ...
            updRet[] :=
                (
                    ...
                    If[occupationFactor =!= Null,
                        AssociateTo[toBeReturned, "OccupationFactor" 
                            -> occupationFactor]
                        ,
                    ...
                    If[AllTrue[coordinates[[1]], NumericQ],
                        AssociateTo[toBeReturned, "FractionalCoordinates"
                             -> coordinates]
                        ,
                    ...
                    Which[
                        (adpType === "Anisotropic") && AllTrue[ADPs[[
                            1]], NumericQ],
                            AssociateTo[toBeReturned, "DisplacementParameters"
                                 -> ADPs];
                    ...
                    If[(element =!= Null) && (AllTrue[coordinates[[1]],
                         NumericQ]),
                        validQ = True
                        ,
                        validQ = False
            ...
            systemParameterFields[system_String] :=
                Which[
                    system === "Triclinic",
                        Column[{Row[{InputField[Dynamic @ a, Number, 
                            FieldHint -> "a", FieldSize -> {5., 1.}, Enabled -> True], Spacer[5],
                             InputField[Dynamic @ b, Number, FieldHint -> "b", FieldSize -> {5., 
                            1.}, Enabled -> True], Spacer[5], InputField[Dynamic @ c, Number, FieldHint
                             -> "c", FieldSize -> {5., 1.}, Enabled -> True]}], Row[{InputField[Dynamic
                             @ \[Alpha], Number, FieldHint -> "\[Alpha]", FieldSize -> {5., 1.}, 
                            Enabled -> True], Spacer[5], InputField[Dynamic @ \[Beta], Number, FieldHint
                             -> "\[Beta]", FieldSize -> {5., 1.}, Enabled -> True], Spacer[5], InputField[
                            Dynamic @ \[Gamma], Number, FieldHint -> "\[Gamma]", FieldSize -> {5.,
                             1.}, Enabled -> True]}]}]
                    ,
                    system === "Monoclinic",
                        Column[{Row[{InputField[Dynamic @ a, Number, 
                            FieldHint -> "a", FieldSize -> {5., 1.}, Enabled -> True], Spacer[5],
                             InputField[Dynamic @ b, Number, FieldHint -> "b", FieldSize -> {5., 
                            1.}, Enabled -> True], Spacer[5], InputField[Dynamic @ c, Number, FieldHint
                             -> "c", FieldSize -> {5., 1.}, Enabled -> True]}], Row[{InputField[\[Alpha]
                             = 90, Number, FieldHint -> "\[Alpha]", FieldSize -> {5., 1.}, Enabled
                             -> False, BaseStyle -> GrayLevel[0.65]], Spacer[5], InputField[\[Beta]
                             = 90, Number, FieldHint -> "\[Beta]", FieldSize -> {5., 1.}, Enabled
                             -> False, BaseStyle -> GrayLevel[0.65]], Spacer[5], InputField[Dynamic
                             @ \[Gamma], Number, FieldHint -> "\[Gamma]", FieldSize -> {5., 1.}, 
                            Enabled -> True]}]}]
                    ,
                    system === "Orthorhombic",
                        Column[{Row[{InputField[Dynamic @ a, Number, 
                            ...
                            1.}, Enabled -> True], Spacer[5], InputField[Dynamic @ c, Number, FieldHint
                             ...
                             -> False, BaseStyle -> GrayLevel[0.65]], Spacer[5], InputField[\[Beta]
                             = 90, Number, FieldHint -> "\[Beta]", FieldSize -> {5., 1.}, Enabled
                             -> False, BaseStyle -> GrayLevel[0.65]], Spacer[5], InputField[\[Gamma]
                             = 90, Number, FieldHint -> "\[Gamma]", FieldSize -> {5., 1.}, Enabled
                             -> False, BaseStyle -> GrayLevel[0.65]]}]}]
                    ,
                    system === "Tetragonal",
                        Dynamic @
                            Column[
                                        ...
                                        Row[
                                            {
                                                ...
                                                Spacer[5]
                                                ,
                                                InputField[b = Dynamic
                                                     @ a, Number, FieldHint -> "b", FieldSize -> {5., 1.}, Enabled -> False,
                                                     BaseStyle -> GrayLevel[0.65]]
                                                ,
                                                Spacer[5]
                                                ,
                                                InputField[Dynamic @ 
                                                    c, Number, FieldHint -> "c", FieldSize -> {5., 1.}, Enabled -> True]
                                            }
                                        ]
                                    ,
                                    Dynamic @ Row[{InputField[\[Alpha]
                                         = 90, Number, FieldHint -> "\[Alpha]", FieldSize -> {5., 1.}, Enabled
                                         -> False, BaseStyle -> GrayLevel[0.65]], Spacer[5], InputField[\[Beta]
                                         = 90, Number, FieldHint -> "\[Beta]", FieldSize -> {5., 1.}, Enabled
                                         -> False, BaseStyle -> GrayLevel[0.65]], Spacer[5], InputField[\[Gamma]
                            ...
                            Column[
                                {
                                    ...
                                    Dynamic @ Row[{InputField[\[Alpha]
                                         ...
                                         = 90, Number, FieldHint -> "\[Beta]", FieldSize -> {5., 1.}, Enabled
                                         -> False, BaseStyle -> GrayLevel[0.65]], Spacer[5], InputField[\[Gamma]
                                         = 120, Number, FieldHint -> "\[Gamma]", FieldSize -> {5., 1.}, Enabled
                                         -> False, BaseStyle -> GrayLevel[0.65]]}]
                                }
                            ]
                    ,
                                        ...
                                        Row[
                                            {
                                                ...
                                                Spacer[5]
                                                ,
                                                InputField[c = Dynamic
                                                     @ a, Number, FieldHint -> "c", FieldSize -> {5., 1.}, Enabled -> False,
                                                     BaseStyle -> GrayLevel[0.65]]
                                            }
                                        ]
                                    ,
            ...
            (* Atom data *)
            updAtomDataSummary[] := atomDataSummary = atomData[[All, 
            ...
            createAtomDataPanel[] :=
                If[deleteButtons === {},
                    atomDataPanel = "(no entries)"
                    ,
                    atomDataPanel = Row[{Pane[Grid[Transpose @ Join[Transpose
                         @ Values @ atomDataSummary, {deleteButtons}], Alignment -> Left], Scrollbars
                         -> False]}]
                ];
            updAtomDataPanel[] :=
                If[atomData === {},
            ...
            gridA =
                Dynamic @
                    {
                        {"Crystal name", InputField[Dynamic @ name, String,
                             FieldHint -> "Crystal name or label"]}
                        ,
                        {
                            "Space group"
                            ,
                                ...
                                Row[
                                    {
                                        PopupMenu[
                                            Dynamic[
                                                ...
                                                (
                                                    ...
                                                    sgSymbol = sgKeys
                                                        [[sgNumber]];
                                                    crystalSystem = $SpaceGroups[
                                                        sgSymbol, "CrystalSystem"];
                                                    parameterFields =
                                        ...
                                        PopupMenu[
                                            Dynamic[
                                                ...
                                                (
                                                    sgSymbol = #;
                                                    sgNumber = $SpaceGroups[
                                                        sgSymbol, "SpaceGroupNumber"];
                                                    crystalSystem = $SpaceGroups[
                                                        sgSymbol, "CrystalSystem"];
                                                    parameterFields =
                                                         systemParameterFields[crystalSystem]
                        ...
                        ,
                        {Tooltip["Lattice parameters", Column[{"a  b  c",
                             "\[Alpha]  \[Beta]  \[Gamma]", "\[ARing]ngstr\[ODoubleDot]m and degree"
                            }], TooltipDelay -> 0.6], Dynamic @ parameterFields}
                        ,
                        {}
                        ,
                        {Tooltip["Chemical formula", "e.g. 'C13 H22 Fe N6 S3'",
                             TooltipDelay -> 0.6], InputField[Dynamic @ chemicalFormula, String, 
                            FieldHint -> "e.g. 'C13 H22 Fe N6 S3'"]}
                        ,
                        {Tooltip["Wavelength", "\[ARing]ngstr\[ODoubleDot]m",
                             TooltipDelay -> 0.6], Row[{InputField[Dynamic @ wavelength, Number, 
                            FieldSize -> {5., 1.}, FieldHint -> "\[Lambda]"], Spacer[5], PopupMenu[
                            Dynamic @ wavelength, {1.54059 -> "Cu \!\(\*SubscriptBox[\(K\[Alpha]\), \(1\)]\)",
                             1.54443 -> "Cu \!\(\*SubscriptBox[\(K\[Alpha]\), \(2\)]\)", 1.39223 
                            -> "Cu \!\(\*SubscriptBox[\(K\[Beta]\), \(1\)]\)", 0.70932 -> "Mo \!\(\*SubscriptBox[\(K\[Alpha]\), \(1\)]\)",
                             0.71361 -> "Mo \!\(\*SubscriptBox[\(K\[Alpha]\), \(2\)]\)", 0.63230 
                            -> "Mo \!\(\*SubscriptBox[\(K\[Beta]\), \(1\)]\)"}, "(Predefined)"]}]
                            }
                        ,
                        {Tooltip["Mass density", "g/\!\(\*SuperscriptBox[\(cm\), \(3\)]\)",
                             TooltipDelay -> 0.6], Row[{InputField[Dynamic @ massDensity, Number,
                             FieldSize -> {5., 1.}, FieldHint -> "\[Rho]"], Spacer[5], "formula units",
                             Spacer[5], InputField[Dynamic @ formulaUnits, Number, FieldSize -> {
                            5., 1.}, FieldHint -> "Z"]}]}
                        ,
                        {}
                        ,
                        {"Atom data", Column[{Button["Add new element",
                             updGrid["B"]], Dynamic @ atomDataPanel}]}
                        ,
                        {}
                        ,
                        {
                            ...
                            Row[
                                {
                                    ...
                                    DefaultButton[
                                        DialogReturn[
                                            ...
                                            (* Return *)
                                            StianRamsnes`MaXrd`Private`$temp
                                                 = {{name, chemicalFormula, formulaUnits, sgSymbol, wavelength}, {a, 
                                                b, c, \[Alpha], \[Beta], \[Gamma]}, atomData};
                                            StianRamsnes`MaXrd`Private`$temp
                                                 = Replace[StianRamsnes`MaXrd`Private`$temp, x_Dynamic :> x[[1]], -1]
                ...
                Dynamic @
                    {
                        ...
                        ,
                        {
                            Tooltip["ADP type", "Anisotropic displacement parameters",
                                 TooltipDelay -> 0.6]
                            ,
                            RadioButtonBar[
                                Dynamic[
                            ...
                            Row[
                                {
                                    ...
                                    DefaultButton[
                                        "Add"
                                        ,
                                        AppendTo[atomData, Replace[toBeReturned,
                                             x_Dynamic :> x[[1]], -1]];
                                        updAtomDataPanel[];
                                        updGrid["A"]
                                        ,
            ...
            (* Grid on display *)
            Dynamic @ Grid[currentGrid[[1]], Spacings -> {1, 0.5}, Alignment
                 -> {Left, Center}]
        ]
        ,
        (* Dialogue settings *)
...
ImportCrystalData[] :=
    Block[{name},
        ...
        If[ListQ @ StianRamsnes`MaXrd`Private`$temp,
            If[!AllTrue[StianRamsnes`MaXrd`Private`$temp[[2]] /. x_Dynamic
                 :> x[[1]], NumericQ],
                Message[ImportCrystalData::latticeParameters];
                Abort[]
                ,
