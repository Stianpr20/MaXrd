...
Options @ SynthesiseStructure = {"Padding" -> False, "RotationAnchorReference"
     -> "DomainCentroid", "RotationAnchorShift" -> {0, 0, 0}, "RotationAxes"
     -> IdentityMatrix[3], "RotationMap" -> <||>, "SelectionMethod" -> "Sequential"
    };

SyntaxInformation @ SynthesiseStructure = {"ArgumentsPattern" -> {_, 
    _., _., OptionsPattern[]}};

Begin["`Private`"];

SynthesiseStructure[blocks_List, {sizeA_Integer, sizeB_Integer, sizeC_Integer
    }, outputName_String, OptionsPattern[]] :=
    Block[
        {selectionMethod = OptionValue["SelectionMethod"], domains, mapping,
             options}
        ,
        (* Checking input *)
        Check[Scan[InputCheck["CrystalEntityQ", #]&, DeleteDuplicates
             @ blocks], Abort[]];
        ...
        domains =
            Which[
                    ...
                    ,
                    True,
                        Message[SynthesiseStructure::InvalidSelectionMethod,
                             selectionMethod];
                        Abort[]
        ...
        options = Thread[# -> OptionValue[#], String]& /@ (First /@ Options
             @ SynthesiseStructure);
        SynthesiseStructure[{{sizeA, sizeB, sizeC}, domains}, outputName,
             mapping, Sequence @@ options]
    ]

SynthesiseStructure[{{inputA_Integer, inputB_Integer, inputC_Integer},
     inputDomains_List}, outputName_String, integerToLabelMap_Association
     : <||>, OptionsPattern[]] :=
    Block[
        {domains = inputDomains, sizeA = inputA, sizeB = inputB, sizeC
             = inputC, blocks, nonVoidRange, normalBlocks, blockSizes, outputSize,
             targetPositions, blockPositionsMap, blockCopy, blockCopies, representativeCellDimensions,
             coordinatesCrystal, coordinatesCartesian, coordinatesCrystalEmbedded,
             newCoordinates, hostMetric, hostMetricInverse, targetPositionsCartesian,
             M, T, anchorShift = OptionValue["RotationAnchorShift"], anchorReference
             = OptionValue["RotationAnchorReference"], rotationAxes = OptionValue[
            "RotationAxes"], rotationMap = OptionValue["RotationMap"], R, twist}
        ,
        (* Checking input *)
        If[TrueQ @ OptionValue["Padding"],
            {{sizeA, sizeB, sizeC}, domains} = InputCheck["PadDomain",
                 {{sizeA, sizeB, sizeC}, inputDomains}]
        ];
        ...
        nonVoidRange = Complement[Range[sizeA * sizeB * sizeC], Flatten
             @ Position[blocks, "Void"]];
        ...
        blockSizes = ($CrystalData[#, "Notes", "StructureSize"] /. _Missing
             -> {1, 1, 1})& /@ blocks;
        If[!SameQ @@ blockSizes,
        ...
        If[(Length @ outputSize != 3) || (AnyTrue[outputSize, Positive[
            #] \[Nand] IntegerQ[#]&]),
        ...
        targetPositionsCartesian = hostMetric . #& /@ targetPositions
            ;
        representativeCellDimensions = GetLatticeParameters[First @ blocks
            ][[ ;; 3]];
        If[rotationMap =!= <||>,
            R = InputCheck["RotationTransformation", {outputSize, domains
                }, {anchorShift, anchorReference, rotationMap, rotationAxes}, representativeCellDimensions,
                 True]
        ];
        blockCopies = Reap[
                Do[
                    M = GetCrystalMetric[blocks[[i]], "ToCartesian" ->
                         True];
                    T = TranslationTransform[targetPositions[[i]]];
                    blockCopy = $CrystalData[blocks[[i]]];
                    coordinatesCrystal = blockCopy[["AtomData", All, 
                        "FractionalCoordinates"]];
                    coordinatesCartesian = M . #& /@ coordinatesCrystal
                        ;
                    coordinatesCrystalEmbedded = hostMetricInverse . 
                        #& /@ coordinatesCartesian;
                    newCoordinates = T /@ coordinatesCrystalEmbedded;
                        
                    If[rotationMap =!= <||>,
                        twist = R[domains[[i]], targetPositionsCartesian
                            [[i]]];
                    ...
                    blockCopy[["AtomData", All, "FractionalCoordinates"
                        ]] = newCoordinates;
                    Sow @ blockCopy
                    ,
                    {i, nonVoidRange}
        ...
        $CrystalData[outputName, "AtomData"] = Flatten @ blockCopies[[
            All, "AtomData"]];
...
SynthesiseStructure[element_String, latticeParameters_List : {5., 5.,
     5., 90., 90., 90.}, spaceGroup_:"P1", inputLabel_String:""] :=
    Block[
        ...
        If[element === "Void",
            ...
            atomData = {<|"Element" -> element, "FractionalCoordinates"
                 -> {0, 0, 0}|>}
        ];
        ...
        ImportCrystalData[{label, chemicalFormula, 0, spaceGroup, -1},
             latticeParameters, atomData, "OverwriteWarning" -> False]
    ]
