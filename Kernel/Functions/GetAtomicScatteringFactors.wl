...
Options @ GetAtomicScatteringFactors = {"DispersionCorrections" -> True,
     "f0Source" -> "WaasmaierKirfel", "f1f2Source" -> "CromerLiberman", "IgnoreIonCharge"
     -> True, "SeparateCorrections" -> False};

SyntaxInformation @ GetAtomicScatteringFactors = {"ArgumentsPattern" 
    -> {_, _, _., OptionsPattern[]}};

Begin["`Private`"];

GetAtomicScatteringFactors[crystal_String, hklInput_List, input\[Lambda]
     : _ ? (NumericQ[#] || QuantityQ[#]&) : -1, OptionsPattern[]] :=
    Block[
        ...
        options = # -> OptionValue[#]& /@ Keys @ Options @ GetAtomicScatteringFactors
            ;
        GetAtomicScatteringFactors[elements, sl, \[Lambda], Sequence 
            @@ options]
    ]

GetAtomicScatteringFactors[inputElements_List, inputSL_List, input\[Lambda]
     : _ ? (NumericQ[#] || QuantityQ[#]&) : -1, OptionsPattern[]] :=
    Block[
        {\[Lambda] = input\[Lambda], elements = inputElements, sl = inputSL,
             f0Source, f1f2Source, $f0Local, $f1f2Local, upperLimit, ignore, addCorrectionsQ
             = TrueQ @ OptionValue["DispersionCorrections"], separateQ = TrueQ @ 
            OptionValue["SeparateCorrections"], ipfQ, coefficients, aKeys, bKeys,
             a, b, c, f0, corrections, output, temp}
        ,
        (*---* Checking input *---*)
        (* Elements *)
        ...
        (* Wavelength *)
        If[\[Lambda] != -1,
            \[Lambda] = QuantityMagnitude @ InputCheck["GetEnergyWavelength",
                 \[Lambda]]
        ];
        (* Data sources *)
        ...
        ignore = {"H", "He"};
        (* Validating sources *)
        If[!MemberQ[FileBaseName /@ FileNames["*.m", FileNameJoin[{$MaXrdPath,
             "Kernel", "Data", "AtomicScatteringFactor"}]], f0Source] || !MemberQ[
            FileBaseName /@ FileNames["*.m", FileNameJoin[{$MaXrdPath, "Kernel", 
            "Data", "AtomicScatteringFactor", "AnomalousCorrections"}]], f1f2Source
            ],
        ...
        Do[
            temp = sl[[i]];
            If[temp > upperLimit,
                Message[GetAtomicScatteringFactors::slRange, ToString
                     @ temp, f0Source];
                Abort[]
            ]
            ,
        ...
        If[(* a. Check if same source is imported already *)KeyExistsQ[
            $f0, f0Source],
            $f0Local = $f0[f0Source]
            ,
            (* b. Import specified data *)
            $f0Local = Import @ FileNameJoin[{$MaXrdPath, "Kernel", "Data",
        ...
        If[temp =!= {},
            Message[GetAtomicScatteringFactors::missing, First @ temp,
                 "non-dispersive part (\!\(\*FormBox[SubscriptBox[\(f\), \(0\)],
TraditionalForm]\))"
                ];
            Abort[]
        ];
        ...
        Which[(* a. Interpolation data *)(* sin(\[Theta])/\[Lambda] *)
            ...
            ipfQ,
                (* Non-dispersive part of form factor *)f0 = Table[{element,
                     $f0Local[element][s]}, {s, sl}, {element, elements}]
            ,
            (* b. Coefficients *)True,
...
and 'c' last *)coefficients = $f0Local[[elements]];
                {aKeys, bKeys} = Flatten @ StringCases[Keys @ First @
                     coefficients, # ~~ DigitCharacter..]& /@ {"a", "b"};
                {a, b, c} = {Values /@ coefficients[[All, aKeys]], Values
                     /@ coefficients[[All, bKeys]], Values @ coefficients[[All, "c"]]};
                (* Non-dispersive part of form factor *)
                f0 = Table[{Keys[coefficients][[i]], Total[a[[i]] * Exp[
                    -b[[i]] * (sl[[j]]) ^ 2]] + c[[i]]}, {j, Length @ sl}, {i, Length @ elements
                    }]
        ];
        ...
        If[Abs[
                If[ipfQ,
                        $f0Local["C"][0]
                        ,
                        Total @ Values @ $f0Local[["C", aKeys]] + $f0Local[
                            "C", "c"]
                    ] - 6(* Carbon: Z=6 *) ] > 0.5,
            f0 = f0 /. {element_String, f_?NumericQ} :> {element, f *
                 $PeriodicTable[element, "AtomicNumber"]}
        ];
        ...
        If[!addCorrectionsQ,
            (* Prepare output *)
            output = Map[Association, MapThread[Rule, Transpose[#]]& 
                /@ f0];
            Goto["End"]
        ];
        ...
        If[(* a. Check if same source is imported already *)KeyExistsQ[
            ...
            ,
            (* b. Import specified data *)
            $f1f2Local = Import @ FileNameJoin[{$MaXrdPath, "Kernel",
                 "Data", "AtomicScatteringFactor", "AnomalousCorrections", f1f2Source
                 <> ".m"}];
...
TraditionalForm]\))"
        ...
        If[separateQ,
            ...
            f0 =
                f0 /.
                    {
                        (* Elements with corrections *){element_String,
                             f0_?NumericQ, f1f2_?NumericQ} :> (element -> <|"f0" -> f0, "f1f2" ->
                             f1f2|>)
                        ,
                        (* Without corrections (ignored elements) *)
                        {element_String, f0_?NumericQ} :> (element ->
                             <|"f0" -> f0, "f1f2" -> 0.|>)
                    }
            ,
            ...
            f0 =
                f0 /.
                    {
                        (* Elements with corrections *){element_String,
                             f0_?NumericQ, f1f2_?NumericQ} :> (element -> f0 + f1f2)
                        ,
                        (* Without corrections (ignored elements) *)
                        {element_String, f0_?NumericQ} :> (element ->
                             f0)
