...
GetElements[input_String, OptionsPattern[]] :=
    Block[
        {formula = input, patternX, groupX, tallyQ = TrueQ @ OptionValue[
            "Tally"], elements, temp}
        ,
        (*---* Check input string *---*)
        ...
        If[MemberQ[Keys @ $CrystalData, input],
            ...
            formula =
                Lookup[
                    ...
                    elements = $CrystalData[[input, "AtomData", All, 
                        "Element"]];
                    If[tallyQ,
                        (* Return tally *)
        ...
        groupX[string_] := StringCases[string, {x:patternX ~~ {n1 : DigitCharacter
            .. ~~ "." ~~ n2 : DigitCharacter..} :> {x, ToExpression[n1 <> "." <> 
            n2]}, x:patternX ~~ n : DigitCharacter.. ~~ pm : {"+", "-"} :> {x, n 
            ~~ pm}, x:patternX ~~ pm : {"+", "-"} ~~ n : DigitCharacter... :> {x,
             n ~~ pm}, x:patternX ~~ n : DigitCharacter... :> {x, ToExpression @ 
            n}}] /. {"" -> "1", Null -> 1};
        (*---* Extracting symbols and numbers *---*)
        (* Distribute parenthesis subscripts *)
        elements = StringReplace[elements, "(" ~~ p__ ~~ ")" ~~ s : DigitCharacter
            .. :> StringJoin[ToString /@ Flatten @ MapAt[# * ToExpression[s]&, groupX
             @ p, {All, 2}]]];
        ...
        If[!DuplicateFreeQ[First /@ elements],
            ...
            elements = elements /. x_ /; Depth[x] === 3 :> {x[[1, 1]],
                 Total @ x[[All, 2]]}
        ];
        (* Checking for ions *)
        elements = elements /. x_List /; Depth[x] == 2 && StringContainsQ[
            ToString @ x[[2]], {"+", "-"}] :> {StringJoin @@ x, "1"};
        (* Optional: Remove charge of ions *)
        If[OptionValue["IgnoreIonCharge"],
            elements = MapAt[StringDelete[#, {DigitCharacter, "+", "-"
                }]&, elements, {All, 1}]
        ];
