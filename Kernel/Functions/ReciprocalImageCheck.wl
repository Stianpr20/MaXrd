...
ReciprocalImageCheck[image_Image, {a_, b_, c_, \[Alpha]_, \[Beta]_, \[Gamma]_}, data_List, Optional[pattern_Condition, {_, _, _} /; False], options : OptionsPattern[{ReciprocalImageCheck, FindPixelClusters, ListPlot}]] :=
    Block[
        {latticeParameters = {a, b, c, \[Alpha], \[Beta], \[Gamma]}, latticeOrigin = OptionValue["LatticeOrigin"], hkl, normalDirection, normalConstant, planeSelection, planeDescriptor, data2D, L, model, \[Lambda]1, \[Lambda]2, h1, h2, \[CapitalLambda], latticeData}
        ,
        (*--- Input check ---*)
        ...
        model = Quiet @ NonlinearModelFit[data2D, Norm[Total[Transpose @ L * {\[Lambda]1, \[Lambda]2} * {h1, h2}] + latticeOrigin], {\[Lambda]1, \[Lambda]2}, {h1, h2}];
...
ReciprocalImageCheck[image_Image, {lattice_List, origin_List, planeDescriptor_List}, Optional[pattern_Condition, {_, _, _} /; False], options : OptionsPattern[{ReciprocalImageCheck, FindPixelClusters, ListPlot}]] :=
    Module[
        {thickness, planeSelection, \[Xi], \[Chi], \[CapitalXi], latticeSize, showGridQ, gridCreator, grid, pixelList, imageDimensions = ImageDimensions @ image, xy2hkl, off, hkl, count, matching, rest, makeTooltip, plotPoints, colorMatch, colorRest, colorOff, optionKeys, plotRange, plotOptions, plot, highlightQ, highlightOverlay}
        ,
        (*--- Input check ---*)
        (* Lattice check *)
        ...
        planeSelection = DeleteCases[planeDescriptor, _?NumericQ] /. <|"h" -> 1, "k" -> 2, "l" -> 3|>;
        (* Image check/treatment *)
        pixelList = FindPixelClusters[image, FilterRules[{options}, Options @ FindPixelClusters]];
        ...
        If[showGridQ,
            gridCreator = RIC$GraphicalLatticeFactory[{lattice, origin, planeSelection}, thickness];
            grid =
        ...
        makeTooltip := KeyValueMap[Tooltip[##, TooltipStyle -> OptionValue["TooltipStyle"]]&, MillerNotationToString /@ #]&;
        ...
        If[plotOptions @ PlotStyle === Automatic,
            plotOptions @ PlotStyle = {{PointSize -> OptionValue @ PointSize, Automatic, colorMatch}, {PointSize -> OptionValue @ PointSize, Automatic, colorRest}, {PointSize -> OptionValue @ PointSize, Automatic, colorOff}}
        ];
        ...
        If[highlightQ,
            highlightOverlay = RIC$GenerateReflectionHighlightOverlay[\[Chi], OptionValue["HighlightReflections"], planeDescriptor, OptionValue["HighlightSymmetry"]];
            highlightOverlay = MapAt[Tooltip[#, MillerNotationToString @ \[CapitalXi] @ First @ #, TooltipStyle -> OptionValue["TooltipStyle"]]&, highlightOverlay, {1, 2, 2 ;; -1 ;; 2, All}]
        ];
        If[TrueQ @ OptionValue["BackgroundImage"],
            (* a. Put invisible nodes on top of image *)
            plotOptions = Normal @ Join[Association @ plotOptions, <|PlotStyle -> {Transparent, PointSize -> Large}, Frame -> False, Axes -> False|>];
            Return @
                Show[
        ...
        If[TrueQ @ OptionValue["StoreDataTemporarily"],
            Return[<|"Image" -> image, "ImageGrayscale" -> ColorConvert[image, "Grayscale"], "ImageNegative" -> ColorNegate[image], "ImageGrayscaleNegative" -> ColorConvert[ColorNegate[image], "Grayscale"], "ImageBinarised" -> Binarize[image], "Lattice" -> {lattice, origin, planeDescriptor}, "PlotData" -> rest, "PlotDataMatch" -> matching, "PlotDataWrong" -> off, "Grids" -> grid|>]
        ];
        Show[
            plot
            ,
...
RIC$LatticeConversionFunctions[lattice_, origin_, threshold_] :=
    Module[{\[CapitalLambda], \[CapitalGamma], tx, ty, \[Xi], \[Chi], convert, residue, result},
        ...
        \[Xi][{x_, y_}] :=
            (
                (* Calculation and discrepancy check *)convert = \[CapitalGamma] . {x - tx, y - ty};
...
RIC$GraphicalLatticeFactory[{lattice_, origin_, planeSelection_}, thickness_] :=
    Module[
        {axisColors, latticeVector1, latticeVector2, latticeSetup, t, tt, makeGrid, u, x1, x2, temp, temp2}
        ,
        (* Preparations *)
        ...
        latticeSetup[x0_, y0_] :=
            (
                ...
                tt = {t, t};
                If[x0 == y0 == 0,
                    {(* Origin arrows *){axisColors[[1]], Arrow[{origin, origin + latticeVector1}]}, {axisColors[[2]], Arrow[{origin, origin + latticeVector2}]}}
                    ,
                    {(* Translated lines *){axisColors[[1]], Dashed, Line[{{0, 0}, latticeVector1} + tt]}, {axisColors[[2]], Dashed, Line[{{0, 0}, latticeVector2} + tt]}}
                ]
            );
        makeGrid[s_] :=
            (
                temp = Table[latticeSetup[i, j], {i, -s, Max[1, s]}, {j, -s, Max[1, s]}];
                temp = Flatten[temp, 2];
                If[s == 0,
                ...
                Graphics[{Thickness @ thickness, temp, AspectRatio -> 1, Axes -> False}]
            );
        makeGrid
    ]

RIC$GenerateReflectionHighlightOverlay[nodeToPixelFunction_, reflections_, planeDescriptor_, symmetry_] :=
    Module[{planeSelection, normalDirection, normalConstant, preferredColors = {Green, Blue, Pink, Cyan, Orange}, colors, hkl, hkl2D, xy, disks, radius = 14, opacity = 0.4},
        planeSelection = DeleteCases[planeDescriptor, _?NumericQ] /. <|"h" -> 1, "k" -> 2, "l" -> 3|>;
        normalDirection = First @ Complement[{1, 2, 3}, planeSelection];
        ...
        colors = Join[preferredColors, ColorData[96, #]& /@ Range[Length @ hkl - Length @ preferredColors]][[ ;; Length @ hkl]];
