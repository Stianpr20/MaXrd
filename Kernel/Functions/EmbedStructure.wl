...
Options @ EmbedStructure = {"DataFile" -> FileNameJoin[{$MaXrdPath, "Kernel", "Data", "UserData", "CrystalData.m"}], "DistortHost" -> None, "DistortionType" -> "Cartesian", "Distortions" -> {0, 0, 0}, "MatchHostSize" -> True, "NewLabel" -> "", "OverlapPrecedence" -> "", "OverlapRadius" -> 1.0, "RotationAnchorReference" -> "Unit", "RotationAnchorShift" -> {0, 0, 0}, "RotationAxes" -> IdentityMatrix[3], "Rotations" -> {0, 0, 0}, "ShowProgress" -> False, "TrimBoundary" -> "None"};

SyntaxInformation @ EmbedStructure = {"ArgumentsPattern" -> {_, _, _, OptionsPattern[]}};

Begin["`Private`"];

EmbedStructure[guestUnitsInput_, targetPositionsInput_List, hostCrystal_String, options : OptionsPattern[{EmbedStructure, DistortStructure}]] :=
    Block[
        {newStructureLabel = OptionValue["NewLabel"], invAbort, conditionFilterQ = False, crystalDataOriginal = $CrystalData, dataFile = OptionValue["DataFile"], hostStructureSize, newSize, probabilities, units, distributionList, i, guestUnits, guestCopies, crystalLabels, specialCrystalLabels, nonVoidRange, matchHostSizeQ = TrueQ @ OptionValue["MatchHostSize"], anchorShift = OptionValue["RotationAnchorShift"], anchorReference = OptionValue["RotationAnchorReference"], R, rotationAxes = OptionValue["RotationAxes"], targetPositions = targetPositionsInput, embedLength, latticeTargetPositions, hostCoordinates, mid, latticeParameters, latticeParametersABC, hostMetric, hostMetricInverse, targetPositionsCartesian, embeddingData, completed, M, T, p, P, CheckAndMakeRuleList, distortions, rotations, performShift, performTwist, conditions, list, shift, twist, MakeAlteration, PrepareAlterationList, conditionedDistortionsQ = False, conditionedRotationsQ = False, coordinatesCrystal, coordinatesCartesian, coordinatesCrystalEmbedded, coordinatesCrystalEmbeddedTranslated, newCoordinates, newCoordinatesCartesian, atomDataHost, atomDataGuest, joinedAtomData, boundary, hostCopy, newUnitCellAtomCount, optionsDS, hostDistorter, findOverlap, intervals, checks, atomData1, atomData2, xyz1, xyz2, nearestList, overlappingCoordinates, overlapPrecedence = OptionValue["OverlapPrecedence"], overlapRadius = OptionValue["OverlapRadius"], tmp}
        ,
        (*--- Input checks ---*)
        ...
        Which[
            ListQ @ guestUnitsInput,
                (* A. 'guestUnits' as list of crystals or elements *)If[guestUnitsInput === {},
                    invAbort[]
                ];
                Which[(* A.a. Regular crystal entries *)
                    AllTrue[guestUnitsInput, StringQ] && (Depth @ guestUnitsInput === 2),
                        Null(* Check complete *) ,
                    (* A.b. Conditional rules *)AllTrue[guestUnitsInput, Head[#] === Rule&] && AllTrue[guestUnitsInput[[All, 1]], Head[#] === Condition&],
            ...
            Head @ guestUnitsInput === Rule,
                (* B. 'guestUnits' as list paired with probabilities *)If[Length @ guestUnitsInput[[1]] != Length @ guestUnitsInput[[2]],
                    invAbort[]
                ];
        ...
        crystalLabels = DeleteDuplicates @ Cases[Flatten[{hostCrystal, guestUnitsInput}], _String, 3];
        Scan[InputCheck["CrystalEntityQ", #]&, crystalLabels];
        specialCrystalLabels = InputCheck["HandleSpecialLabels", crystalLabels];
        overlapRadius = overlapRadius / GetLatticeParameters[hostCrystal, "Units" -> False][[{1, 2, 3}]];
        ...
        If[OptionValue["DistortHost"] =!= None,
            ...
            optionsDS = Normal @ Append[Association @ optionsDS, {"DistortionType" -> OptionValue["DistortionType"], "ReturnConverter" -> True}];
        ...
        If[matchHostSizeQ && hostStructureSize =!= {0, 0, 0},
            ...
            targetPositions = Flatten[Outer[Plus, latticeTargetPositions, targetPositions, 1], 1];
            targetPositions = DeleteCases[targetPositions, {x_, y_, z_} /; Or @@ MapThread[Greater, {{x, y, z}, hostStructureSize}]];
        ...
        guestUnits =
            Which[
                conditionFilterQ,
                    targetPositions /. Append[guestUnitsInput, {x_, y_, z_} /; True -> "Void"]
                ,
                Head @ guestUnitsInput === Rule,
                    ...
                    RandomSample @ Flatten @ MapThread[ConstantArray, {units, distributionList}]
                ,
                True,
                    PadRight[#, embedLength, #]& @ guestUnitsInput
            ];
        guestCopies = $CrystalData /@ guestUnits;
        nonVoidRange = Complement[Range @ embedLength, Flatten @ Position[guestUnits, "Void"]];
        If[nonVoidRange === {},
            Goto["End"]
        ];
        latticeParameters = GetLatticeParameters[hostCrystal, "Units" -> False];
...
(*--- Distortions and rotations -- Checks and preparations ---
    *)
        ...
        R =
            If[performTwist,
                InputCheck["RotationTransformation", {{0, 0, 0}, {}}, {anchorShift, anchorReference, {}, rotationAxes}]
            ];
        completed = 0;
        ...
        joinedAtomData =
            If[!MemberQ[{"Host", "Guest"}, overlapPrecedence],
                ...
                overlappingCoordinates = Flatten[MapThread[findOverlap, {xyz1, nearestList}], 1];
                atomData2 = DeleteCases[atomData2, x_ /; MemberQ[overlappingCoordinates, x["FractionalCoordinates"]]];
        ...
        If[boundary =!= "None",
            Which[
                boundary === "Box",
                    joinedAtomData = DeleteCases[joinedAtomData, {x_, y_, z_} /; Nand @@ MapThread[0 <= #1 < #2&, {{x, y, z}, hostStructureSize}], {2}]
                ,
                boundary === "OuterEdges",
                    joinedAtomData = DeleteCases[joinedAtomData, {x_, y_, z_} /; Nand @@ MapThread[0 <= #1 < #2&, {{x, y}, hostStructureSize[[{1, 2}]]}], {2}]
            ];
            joinedAtomData = Select[joinedAtomData, KeyExistsQ[#, "FractionalCoordinates"]&]
        ];
        joinedAtomData = MapAt[N @ Chop[#, 10. ^ -6]&, joinedAtomData, {All, "FractionalCoordinates"}];
...
ES$InterpretPermutations[command_List, embeddingData_Association] :=
    Block[{UnfoldAmplitudes, guestUnits, targetPositions, length, amplitudes, rules},
