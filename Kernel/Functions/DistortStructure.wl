...
Options @ DistortStructure = {"DistortionType" -> "Crystallographic", "NewLabel" -> "", "ReturnConverter" -> False};

SyntaxInformation @ DistortStructure = {"ArgumentsPattern" -> {_, _, OptionsPattern[{CrystalPlot, VectorPlot3D}]}};

Begin["`Private`"];

DistortStructure[crystal_, function_, OptionsPattern[]] :=
    Module[
        {newLabel = OptionValue["NewLabel"], distortionType = OptionValue["DistortionType"], coordinateConverter, distortions, crystalMetric, inverseM, coordinates, coordinatesCartesian, newCoordinates, crystalCopy}
        ,
        (* Input checks *)
        ...
        coordinateConverter[coordinates_] :=
            (
                ...
                If[distortionType === "Cartesian",
                    crystalMetric = GetCrystalMetric[crystal, "ToCartesian" -> True];
